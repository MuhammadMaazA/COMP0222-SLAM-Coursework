% Q2c: Full SLAM with many landmarks — chi2, timing and error analysis
%
% This script runs both G2O and EKF SLAM on a larger scenario with ~200
% landmarks. It produces:
%   Figure 1: Live simulation view
%   Figure 2: Trajectory comparison with landmark positions
%   Figure 3-4: State estimation errors per estimator (x, y, psi)
%   Figure 5: G2O vs EKF state discrepancy
%   Figure 6: Chi2 and optimisation time trends
%   Console: RMSE, consistency, and comparison with Q1b trends
...
% -----------------------------------------------------------------
ebe.graphics.FigureManager.getFigure('Q2c: Trajectory Comparison');
clf

plot(lmMat(:,1), lmMat(:,2), 'k^', 'MarkerSize', 5, ...
...
title('Q2c: Trajectory with Landmarks', 'FontSize', 14)
...
if numel(resultsAccumulator.xEstStore) >= 2
    ...
    ebe.graphics.FigureManager.getFigure('Q2c: G2O vs EKF Comparison');
    clf
    ...
    sgtitle('Q2c: G2O $-$ EKF State Discrepancy', ...
...
% -----------------------------------------------------------------
% Console: trend analysis and comparison to Q1b
% -----------------------------------------------------------------
fprintf('===== Q2c: Chi2 and Timing Analysis =====\n');
...
fprintf('\n--- Observations (Comparison with Q1b) ---\n');
fprintf('1. Chi2 grows more steeply than Q1b because each landmark\n');
fprintf('   observation adds a range-bearing edge with TWO residuals\n');
fprintf('   (range + bearing), contributing more cost terms per step.\n');
fprintf('2. Optimisation time grows MUCH faster than Q1b because the\n');
fprintf('   Hessian now includes landmark state vertices (2 DOF each).\n');
fprintf('   With ~200 landmarks, the state dimension is >> Q1b.\n');
fprintf('3. The Hessian sparsity pattern is denser (landmarks connect\n');
fprintf('   to multiple platform poses), making each solve more expensive.\n');
fprintf('4. Both chi2 and timing still show the same fundamental trend\n');
fprintf('   as Q1b: monotonic growth driven by the ever-increasing graph.\n');
fprintf('   This motivates graph pruning/marginalisation (Q3).\n');
fprintf('==========================================\n\n');
